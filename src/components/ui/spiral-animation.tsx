...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      let easingValue: number;
      if (displacementProgress < 0.3) {
        easingValue = controller.lerp(linearEasing, powerEasing, displacementProgress / 0.3);
      } else if (displacementProgress < 0.7) {
        const t = (displacementProgress - 0.3) / 0.4;
        easingValue = controller.lerp(powerEasing, elasticEasing, t);
      } else {
        easingValue = elasticEasing;
      }
      ...
      if (displacementProgress < 0.3) {
        screenX = controller.lerp(spiralPos.x, spiralPos.x + this.dx * 0.3, easingValue / 0.3);
        screenY = controller.lerp(spiralPos.y, spiralPos.y + this.dy * 0.3, easingValue / 0.3);
      } else if (displacementProgress < 0.7) {
