  ...
  get count() {
    ...
    get coords(): [number, number] {
      return this.pointers.size > 0
        ? (Array.from(this.pointers.values()).flat() as [number, number])
        : [0, 0];
    }
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		col=mix(col,vec3(bg*.08,bg*.03,bg*.22),d);
	}
	col *= vec3(0.8,0.6,1.25);
	O=vec4(col,1);
}`;
...
export function AnimatedShaderBackground({ className }: { className?: string }) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn(
        "absolute inset-0 w-full h-full object-contain touch-none pointer-events-none",
        className
      )}
      style={{ background: "black" }}
    />
